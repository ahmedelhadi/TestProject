    ...
    if(!function_exists('load_theme_styles')){
        function load_theme_styles(){
            if (!is_admin()){
                ...
                wp_register_style('ekko-lightbox',  get_template_directory_uri() . '/css/ekko-lightbox.css', array(), '1.0', 'all');
                wp_register_style('responsive-tabs',  get_template_directory_uri() . '/css/responsive-tabs.css', array(), '1.0', 'all');
    ...
    if(!function_exists('load_theme_scripts')){
        function load_theme_scripts(){
            if (!is_admin()) {
                ...
                wp_register_script('ekko-lightbox', $java_script_url.'ekko-lightbox.js', array('jquery'),'1.2.1', false);
                wp_register_script('jquery-responsiveTabs', $java_script_url.'jquery.responsiveTabs.js', array('jquery'),'1.2.1', false);
...
/*	Property Price Format
/*-----------------------------------------------------------------------------------*/
if(!function_exists('get_property_price2')){
    function get_property_price2($id){
//        global $post;
        $price_digits = doubleval(get_post_meta($id, 'REAL_HOMES_property_price', true));
        if($price_digits){
            $currency = get_theme_currency();
            $price_post_fix = get_post_meta($id, 'REAL_HOMES_property_price_postfix', true);
            $decimals = intval(get_option( 'theme_decimals'));
            $decimal_point = get_option( 'theme_dec_point' );
            $thousands_separator = get_option( 'theme_thousands_sep' );
            $currency_position = get_option( 'theme_currency_position' );
...
if(!function_exists('property_price2')){
    function property_price2($id){
        echo get_property_price2($id);
    }
}
...
if(!function_exists('advance_search_options')){
    function advance_search_options($taxonomy_name){
        ...
        if($searched_term == 'any' || empty($searched_term)){
            echo '<option value="any" selected="selected">'.__( 'بعض', 'framework').'</option>';
        } else {
            echo '<option value="any">'.__( 'بعض', 'framework').'</option>';
        }
    }
}
...
add_shortcode('krPropertiesCarousel','krPropertiesCarousel');

function krPropertiesCarousel($atts){
	...
	if(count($categories)>0){
		?>
		<div style="background-color:#fcfdfd;border:0px solid #ccc;padding:0px;margin:0px; overflow: hidden;">
			<div class="krPropertyStatus">
				<a href="#" status="status_for_sale" class="active" style="margin-top:110px;">للبيع</a>
				<a href="#" status="status_for_rent">للإيجار</a>
			</div>
			<div class="krPropertyListing">
				<ul id="krCategoriesList" style="margin-left:22px !important">
				<?php
				foreach($categories as $cat){
					$ccat = get_term_by('id',$cat,'property-type');
					$catName = $ccat->name;
					$args = array(
						'posts_per_page'=>-1,
						'post_type'=>'property',
					...
					if($posts){
						...
						$len = strlen($catName);
						if($len>13){
							$str1 = 'style="line-height:20px"';
						}
						if($ccount==1){
							$str = 'categoryTitleActive';
						}
						?>
						<li>
							<span class="categoryTitle <?=$str ?>" <?=$str1 ?>><?=$catName ?></span>
							<div class="krCategoryPosts" style="display:none">
								<ul>
								<?php
								foreach($posts as $post){
									$thid = get_post_thumbnail_id( $post->ID ); 
									$imgAttr = wp_get_attachment_image_src( $thid, 'full' );
									$thumb = $imgAttr[0];
									$title = get_the_title($post->ID);
									$area = get_post_meta($post->ID,'REAL_HOMES_property_size',true).' '.get_post_meta($post->ID,'REAL_HOMES_property_size_postfix',true);
									$price = get_post_meta($post->ID,'REAL_HOMES_property_price',true).' ريال '.get_post_meta($post->ID,'REAL_HOMES_property_price_postfix',true);
									$meta = '<div class="property-meta">';

									$bedrooms = get_post_meta($post->ID,'REAL_HOMES_property_bedrooms',true);

									if($bedrooms!=''){

										$meta .= '<span style=" font-size: 13px; padding-right: 5px; "><i class="icon-bed"></i>'.$bedrooms.'&nbsp;غرف نوم</span>';

									}

									$bathrooms = get_post_meta($post->ID,'REAL_HOMES_property_bathrooms',true);

									if($bathrooms!=''){

										$meta .= '<span style=" font-size: 13px; padding-right: 5px; "><i class="icon-bath"></i>'.$bathrooms.'&nbsp;دورات مياه</span>';
									...
									if($garage!=''){
										...
										$meta .= '<span style=" font-size: 13px; padding-right: 5px; "><i class="icon-garage"></i>'.$garage.'&nbsp;كراج</span>';

									}

									$meta .= '</div>';
									$link = get_permalink($post->ID);
									$term_list = wp_get_post_terms($post->ID, 'property-status', array("fields" => "names"));
									$status = str_ireplace(" ","_",strtolower($term_list[0]));
									if($ccount==1 && $status=='for_sale'){
										ob_start();
									}
									$statuses = array("for_sale"=>"للبيع","for_rent"=>"للإيجار");
									?>
										<li class="status_<?=$status ?>">
											<a href="<?=$link ?>"><img class="thumb" width="327" height="246" src="<?=$thumb ?>" alt="<?=$title ?>" /></a>
											<div class="propertyDetail">
												<div class="propertyArea">
													<span>قياس : </span><?=$area ?>
												</div>
												<div class="propertyPrice">
													<span>السعر: </span><?=$price ?>
												</div>
												<div class="propertyStatus"><?=$statuses[$status] ?></div>
												<?=$meta ?>
												<a href="<?=$link ?>" class="readmore more-details12"><i class="fa fa-caret-left"></i> تفاصيل أكثر</a>
											</div>
										</li>
									<?php
									if($ccount==1 && $status=='for_sale'){
				...
				<div class="propertyCarousel">
					<ul class="bxslider" style="margin-left:0px !important"><?=$postsList ?></ul>
				</div>
			</div>
			<div style="clear:both;margin-bottom:10px"></div>
			...
			<script>
				jQuery(document).ready(function(){
					var propSlider = jQuery('.bxslider').bxSlider({
						slideWidth: 327,
						minSlides: 2,
						maxSlides: 3,
						slideMargin: 14,
					...
					jQuery('#krCategoriesList .categoryTitle').click(function(){
						jQuery('#krCategoriesList .categoryTitle').removeClass('categoryTitleActive');
						jQuery(this).addClass('categoryTitleActive');
						var status = jQuery('.krPropertyStatus a.active').attr('status');
						var newSliders = '';
						jQuery(this).next().find('.'+status).each(function(){
							newSliders += '<li>'+jQuery(this).html()+'</li>';
						});
						jQuery('ul.bxslider').html(newSliders);
						propSlider.reloadSlider({
							slideWidth: 327,
							minSlides: 2,
							maxSlides: 3,
							slideMargin: 14,
					...
					jQuery('.krPropertyStatus a').click(function(evt){
						evt.preventDefault();
						jQuery('.krPropertyStatus a').removeClass('active');
						jQuery(this).addClass('active');
						var ccat = jQuery('#krCategoriesList .categoryTitleActive');
						var status = jQuery(this).attr('status');
						var newSliders = '';
						ccat.next().find('.'+status).each(function(){
							newSliders += '<li>'+jQuery(this).html()+'</li>';
						});
						jQuery('ul.bxslider').html(newSliders);
						propSlider.reloadSlider({
...
add_action('save_post', 'ib_save_postdata' );
function ib_save_postdata($post_id){
	...
	if ('property'==$_POST['post_type']){
		if ( !current_user_can( 'edit_page', $post_id ) ){
			...
			$cityname = substr($terms[0]->slug,0,1);
			$dir = substr(get_post_meta($post_id,'direction',true),0,1);
			$pid = $cityname.$dir.$post_id;
			update_post_meta($post_id,'REAL_HOMES_property_id',$pid);
