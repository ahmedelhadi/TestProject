...
if( ($display_agent_info == 'true') && ($agent_display_option != "none") ){
    ...
    if($agent_display_option == "my_profile_info"){
        ...
        $agent_title_text = __('أضيف بواسطة ','framework')." ".get_the_author_meta('display_name');
        ...
        if( ( !empty($property_agent) ) && ( intval($property_agent) > 0 ) ){
            ...
            $agent_title_text = __('وكيل','framework')." ".get_the_title($agent_id);
        ...
        <div class="agent-detail clearfix">
            ...
            <div class="left-box">
                ...
                <ul class="contacts-list">
                    ...
                    if(!empty($agent_office_phone)){
                        ?><li class="office"><?php _e('مكتب', 'framework'); ?> : <?php echo $agent_office_phone; ?></li><?php
                    }
                    if(!empty($agent_mobile)){
                        ?><li class="mobile"><?php _e('موبايل', 'framework'); ?> : <?php echo $agent_mobile; ?></li><?php
                    }
                    if(!empty($agent_office_fax)){
                        ?><li class="fax"><?php _e('الفاكس', 'framework'); ?>  : <?php echo $agent_office_fax; ?></li><?php
                    }
                    ?>
                </ul>
                <p>
                    ...
                    if($display_author){
                        ...
                        ?>
                        <br/>
                        <a class="real-btn" href="<?php echo get_permalink($agent_id); ?>"><?php _e('معرفة المزيد','framework'); ?></a>
                        <?php
                    }
                    ?>
                </p>
            ...
            if(!empty($agent_email)){
                ...
                <div class="contact-form">
                    ...
                    <h3><?php _e('التواصل مع الوكيل " المسوق " ', 'framework'); ?></h3>

                    <form id="agent-contact-form" class="contact-form-small" method="post" action="<?php echo site_url(); ?>/wp-admin/admin-ajax.php">

                        <input type="text" name="name" id="name" placeholder="<?php _e('الاسم', 'framework'); ?>" class="required" title="<?php _e('* Please provide your name', 'framework'); ?>">

                        <input type="text" name="email" id="email" placeholder="<?php _e('البريد الالكتروني', 'framework'); ?>" class="email required" title="<?php _e('* Please provide valid email address', 'framework'); ?>">

			<input type="text" name="phone" id="phone" placeholder="<?php _e('رقم الجوال', 'framework'); ?>" class="phone required" title="<?php _e('* Please provide valid Phone number', 'framework'); ?>">

                        <textarea  name="message" id="comment" class="required" placeholder="<?php _e('الرسالة', 'framework'); ?>" title="<?php _e('* Please provide your message', 'framework'); ?>"></textarea>
                        ...
                        <input type="submit" value="<?php _e('إرسال','framework'); ?>"  name="submit" class="real-btn">
